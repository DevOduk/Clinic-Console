...
1. The components you identified and how you divided the screen up.
2. Where each piece of state lives, and why there rather than somewhere else. Server data, URL state and local UI state are not
the same thing.
3. How you fetch, cache and invalidate data.
4. Your approach to layout, spacing, colour and typography. If you are using design tokens or a theme, say so. If you are using a
library’s defaults, say that instead.
5. Your accessibility approach.
...
1. Typing in the search box must never leave the user looking at results for a query they have already replaced, even on a slow
connection. Verify this yourself using the delay parameter.
2. Changing the category filter or the sort order must not strand the user on an empty page.
3. Reloading the browser must put the user back where they were: same search, same filter, same sort, same page. So must
opening a copied URL on another machine.
4. Every screen that loads data has a loading state, an empty state and an error state. The error state must offer the user a way to
recover. Test the error path against /http/500.
5. The whole app is usable with a keyboard alone, and readable at 360px wide.
...
1. What did you use AI for across the four sections? Answer per section, not in general.
2. Which tools did you use? Include any spec-driven development or agent workflow framework, for example Superpowers, GSD,
...
5. You walk us through your own code. We will pick sections and ask why they are the way they are, and what breaks if we
remove them.
6. We share a short component with you and ask you to review it.
7. We describe a production incident and ask you to work out what is happening.
8. We ask you to sketch or start a small extension to what you built.
